...
//refreshToken은 accessToken이 만료되었을때 재발급할 수 있도록 사용하는 토큰임
...
app.get("/set-token/:id", (req, res) => {
  ...
  const accessToken = createAccessToken(id); //실제로 사용자가 서버에 접근을 할때 만들어지는 토근
  const refreshToken = createRefreshToken(); // 현재 사용자가 서버에서 인증받은 사용자가 맞는지 검증하는 토큰
...
function createRefreshToken() {
  const refreshToken = jwt.sign(
    {}, // JWT 데이터 원래 사용하던 키를 가지고 오는거라 빈칸임
    SECRET_KEY, // 비밀키
    { expiresIn: "7d" }
...
app.get("/get-token", (req, res) => {
  const accessToken = req.cookies.accessToken; //가지고 있던거 할당
  ...
  const isAccessTokenValidate = validateAccessToken(accessToken); //함수로 분리해서 확인
  ...
  if (!isAccessTokenValidate) {
    // 위에서 tokenObject[refreshToken] = id; 이거 해준거 사용
    const accessTokenId = tokenObject[refreshToken];
    //토큰이 탈취되어서 우리가 Refresh 토큰을 삭제했을때
    if (!accessTokenId)
      return (
        res
          .status(419)
          //Refresh Token이 인증되었지만, 서버에 존재하지 않을 때 에러
  ...
  const { id } = getAccessTokenPayload(accessToken); //객체 구조분해 할당
  return res.json({
